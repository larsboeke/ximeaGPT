...
def create_chat(user_id, user_prompt):
    ...
    entry = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a helpful assistant to the customer support in the Company XIMEA. Base your Answers as much as possible on information gathered by the functions. If you are unsure always use functions!",
            }
        ],
    }
    ...
    test = user_mongo.update_one(
        {"user_id": user_id},
        {
            "$push": {
                "conversations": {"conversation_id": conversation_id, "title": title}
            }
        },
    )
    print(test)
...
def generate_chat_title(user_prompt):  #
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Your are a assiatant that generates descriptive Titlesof Chats based on the first User Message. ONLY CREATE TITLES THAT CONTAIN LESS THAN 3 WORDS. DO NOT USE MORE THAN THREE WORDS!",
            },
...
def delete_chat(user_id, chat_id):
    ...
    user_mongo.update_one(
        {"user_id": user_id}, {"$pull": {"conversations": {"conversation_id": chat_id}}}
    )
...
def add_message(conversation_id, role, content):
    ...
    conversations_mongo.update_one(
        {"conversation_id": conversation_id}, {"$push": {"messages": message}}
    )
    print("added message")


def drop_message(conversation_id):
    conversations_mongo.update_one(
        {"conversation_id": conversation_id}, {"$unset": {f"messages.1": ""}}
    )
    conversations_mongo.update_one(
        {"conversation_id": conversation_id}, {"$unset": {f"messages.2": ""}}
    )
    conversations_mongo.update_one(
        {"conversation_id": conversation_id}, {"$pullAll": {"messages": [None]}}
    )
...
def add_function(conversation_id, function_name, content):
    ...
    conversations_mongo.update_one(
        {"convesation_id": conversation_id}, {"$set": {"messages": updated_messages}}
    )
...
def retrieve_conversation(conversation_id):
    ...
    conversation = [
        {k: v for k, v in msg.items() if k != "timestamp" and k != "sources"}
